...
def betting_stats_by_league(df: pd.DataFrame):
    ...
    for match in df["match_name"].unique():
        ...
        for i, snap in match_snaps.iterrows():
            ...
            if home_goals != prev_home or away_goals != prev_away:  # goal detected
                # Get the *next* odds snapshot (after goal)
                if i + 1 < len(match_snaps):
                    post_snap = match_snaps.iloc[i + 1]
                else:
                    post_snap = snap  # fallback if last row
                    ...
                    all_events.append({
                        ...
                        "to_score": snap["score"],
                        "post_handicap": post_snap["hh_value"],   # numeric handicap (home side)
                        "post_ah": post_snap["ah_value"],         # numeric handicap (away side)
                        "post_line": post_snap["line_value"],     # numeric O/U line
                    })
    ...
    # --- Aggregate stats ---
    stats = (
        events_df.groupby(["country", "league", "post_handicap", "from_score", "to_score"])
        .size()
        .reset_index(name="count")
    )
    stats["total_for_handicap"] = stats.groupby(["country", "league", "post_handicap"])["count"].transform("sum")
    ...
    handicap_stats["total_for_fromscore_handicap"] = handicap_stats.groupby(
        ["country", "league", "post_handicap", "from_score"]
    )["count"].transform("sum")
...
if __name__ == "__main__":
    ...
    sql =     """
        ...
        WHERE "run_time"::TIMESTAMP >= (NOW()::timestamp) - INTERVAL '15000 hours'
   ...
   ## df.rename(columns={'l': "country", "n": "league"}, inplace=True)
    ...
    with pd.ExcelWriter("betting_stats_post.xlsx", engine="openpyxl") as writer:
    ...
    print("✅ Excel file saved as betting_stats_post.xlsx")